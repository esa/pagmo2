...
public:
     ...
     */
    lennard_jones(unsigned atoms = 3u) : m_atoms(atoms)
    {
        if (atoms < 3) {
...
private:
    ...
    double _r(unsigned atom, unsigned coord, const vector_double &x) const
    {
...
PAGMO_REGISTER_PROBLEM(pagmo::lennard_jones)
