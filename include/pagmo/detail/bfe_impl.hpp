...
template <typename P>
inline void bfe_check_input_dvs(const P &p, const vector_double &dvs)
{
...
template <typename P>
inline void bfe_check_output_fvs(const P &p, const vector_double &dvs, const vector_double &fvs)
{
    const auto n_dim = p.get_nx();
    const auto n_dvs = dvs.size() / n_dim;
