...
{
public:
    /// Single entry of the log (gen, fevals, ideal_point ... FIX THIS LIST, what is what? )
    typedef std::tuple<unsigned, double, double, double, double, double, double, double> log_line_type;
    /// The log
     ...
     * positive integer, \p evalstop is not a positive integer, \p focus is not \f$ \in [0,1[\f$, \p ants is not a
     * positive integer, \p ker is not a positive integer, \p oracle is not positive, \p paretomax is not a positive
     * integer, \p epsilon is not \f$ \in [0,1[\f$
     */
    gaco(unsigned gen = 100u, double acc = 0.95, double fstop = 0.0000001, int impstop = 100, int evalstop = 100,
         double focus = 0, unsigned ker = 600, double oracle = 10000, unsigned paretomax = 10, double epsilon = 0.9,
    ...
    population evolve(population pop) const
    {
        ...
        for (decltype(m_gen) gen = 1u; gen <= m_gen; gen++) {
            ...
            // I otherwise proceed with a single-objective algorithm:
                ...
                // The following returns a vector of vectors in which objectives, equality and inequality constraints
                // are concatenated,for each individual
                auto fit = pop.get_f();
                ...
                // note that pop.get_x()[n][k] goes through the different individuals of the population (index n) and
                // the number of variables (index k) the number of variables can be easily be deducted from counting the
                // bounds.

                // I verify whether the maximum number of function evaluations or improvements has been exceeded, if yes
                // I return the population and interrupt the algorithm
                ...
                for (decltype(NP) i = 0u; i < NP; ++i) {
                    ...
                    // case that it is different than zero
                    ...
                    // I verify that the equality and inequality constraints make the solutions feasible, if not, they
                    // are discarded:
                    ...
                    for (decltype(NIC) i_nic = 1u; i_nic <= NIC && T_2 == 0; ++i_nic) {
                        if (fit[i][i_nic]
                            >= -m_acc) { // remember that the ineq constraints are of the kind: g_ineq(X)>=0
                            T_2 = 1;
                        }
                    }
                    if (T == 0 & T_2 == 0) {
                ...
                if (feasible_set >= 1) {
                    ...
                    for (decltype(penalties.size()) j = 0u; j < penalties.size(); ++j) {
                        ...
                        for (decltype(penalties.size()) i = 0u; i < penalties.size() && count == 0; ++i) {
                            if (sorted_penalties[j] == penalties[i]
                                && sorted_penalties[j] != std::numeric_limits<double>::infinity()) {
                                if (j == 0) {
                                    ...
                                    // with the following piece of code I avoid to store the same position in case that
                                    // two another element exist with the same value
                                    int count_2 = 0;
                                    for (decltype(sort_list.size()) jj = 0u; jj < sort_list.size() && count_2 == 0;
                                         ++jj) {
                                        if (sort_list[jj] == i) {
                                            count_2 = 1;
                                        }
                    ...
                    if (gen == 1) {
                        ...
                        if (feasible_set < m_ker) {
                            pagmo_throw(std::invalid_argument,
                                        " Error: the initial population does not have at least m_ker feasible "
                                        "individuals to be stored in the solution archive ");
                        }

                        // I initialize the solution archive (SA): in order to do this I store the vectors generated in
                        // the first generation by taking into account their penalty values. In this way, the first
                        // vector in the SA (i.e., the first row in which penalty value, variables, objective functions
                        // values, equality constraints values, inequality constraints values are stored) represents the
                        // best one (i.e., the one that has the smallest penalty function value among the individuals of
                    ...
                    if (m_verbosity > 0u) {
                        ...
                        if (gen % m_verbosity == 1u || m_verbosity == 1u) {
                            double best_penalty = SA[0][0];
                            double best_fit = SA[0][1 + dim];
                            double worst_penalty = SA[m_ker - 1][0];
                            double worst_fit = SA[m_ker - 1][1 + dim];
                            double best_x = SA[0][1];
                            double best_y = SA[0][2];
                            double dx = 0., df = 0.;
                            ...
                            if (count_screen > 0) { // count_screen % 50u == 1u) {
                                print("\n", std::setw(7), "Gen:", std::setw(15), "Best:", std::setw(15),
                                      "Best penalty:", std::setw(15), "Worst:", std::setw(15), "Best X:", std::setw(15),
                                      "Best Y:", std::setw(15), "Worst penalty:", std::setw(15),
                                      "Oracle:", std::setw(15), "dx:", std::setw(15), std::setw(15), "df:", '\n');
                            }
                            print(std::setw(7), gen, std::setw(15), best_fit, std::setw(15), best_penalty,
                                  std::setw(15), worst_fit, std::setw(15), best_x, std::setw(15), best_y, std::setw(15),
                                  worst_penalty, std::setw(15), m_oracle, std::setw(15), dx, std::setw(15), df, '\n');
                            ++count_screen;
                            // Logs
                    ...
                    // 4 - use pheromone values to generate new ants, which will become the future generation's
                    // variables here you have to define a probability density function and use a random number
                    // generator to produce the new ants from it

                    // I create the vector of vectors where I will store all the new ants (i.e., individuals) which will
                    // be generated
                    ...
                    if (SA[0][1 + dim] < m_oracle && res == 0) {
                        ...
                        for (int rows = 0u; rows < m_ker; ++rows) {
                            ...
                            if (FITNESS > m_oracle && res < diff / 3.0) {
                                alpha
                                    = (diff * (6.0 * std::sqrt(3.0) - 2.0) / (6.0 * std::sqrt(3)) - res) / (diff - res);
                    ...
                    pagmo_throw(std::invalid_argument, " Error: the population does not have any feasible individuals "
                                                       "to be compared with the solution archive  ");
                }
            }
...
private:
    ...
    double penalty_computation(const vector_double &f, const population &pop) const
    {
         ...
         * @param[in] SA Solution archive: the solution archive is useful for retrieving the current individuals (which
         * will be the means of the new pdf)
         */
    ...
    // in the next function I need: 1) the number of the population --> which I will call n_con, 2) the solution archive
    // (where I should place the variables, the obj function
    // the constraints violations and the penalty function values inside). The size of the SA is assumed to be defined
    // as K
    void pheromone_computation(vector_double &OMEGA, vector_double &SIGMA, const population &popul,
                               const unsigned generations,
                               std::vector<vector_double> &SA) const // the size of OMEGA is K, of SIGMA is n_con
    {
         ...
         * @param[in] SIGMA Sigma: the standard deviations are passed to be modified (i.e., they are one of the
         * pheromone values)
        ...
        for (decltype(n_con) h = 1; h <= n_con; ++h) {
            ...
            // at first I define D_min using the subtraction of the first two individuals of the same variable stored in
            // the SA
    ...
    void update_SA(const population &pop, vector_double &sorted_vector, std::vector<double> &sorted_list,
                   int &N_impstop, int &N_evalstop, std::vector<vector_double> &SA) const
    {
        /**
        ...
        // sorted_list contains the position values of these penalties wrt their original position as it appears in
        // get_x()
        auto variables = pop.get_x();
        auto objectives = pop.get_f();

        // note that pop.get_x()[n][k] goes through the different individuals of the population (index n) and the number
        // of variables (index k) the number of variables can be easily be deducted from counting the bounds.

        // I now re-order the variables and objective vectors (remember that the objective vector also contains the eq
        // and ineq constraints):
        ...
        // now I have the individuals sorted in such a way that the first one is the best of its generation, and the
        // last one the worst I can thus compare these individuals with the SA: if for instance the first individual of
        // the sorted generation is worse than the last individual of the SA, then all the others will also be worse,
        // and I can thus interrupt the update. The same holds for the following elements

        // I assume that SA has NP rows (which you call K in the lit. study) and n_con columns: so as many rows as the
        // number of individuals and as many columns as the number of variables for each individual
    ...
    void generate_new_ants(const population &pop, vector_double omega, vector_double sigma, int n_con,
                           std::vector<vector_double> &X_new, int pop_size, double gen,
                           std::vector<vector_double> &SA) const
    {
        /**
        ...
        // I hereby generate the new ants based on a multi-kernel gaussian probability density function. This pdf is
        // nothing more than a weighted sum of several gaussian pdf hence, in order to reproduce it, we first compute
        // the numbers generated by the various gaussian pdf (using the mean and standard deviation values), and we then
        // weight them over the weights (omega) --> weights and stddev were computed in the pheromone_computation
        // function, whereas the mean of the various gaussian pdf is directly related to the previous individuals (i.e.,
        // indeed, their exact value is assumed to be the mean)

        // X_new is the vector of vectors where I will store all the generated ants

        // the number of individuals which are contained in the solution archive is K, whereas I assume to pass the
        // number of different variables which are to be optimized:
        for (decltype(pop_size) j = 0u; j < pop_size; ++j) {
            ...
            for (decltype(n_con) h = 0u; h < n_con; ++h) {
                ...
                for (decltype(SA.size()) k = 0u; k < SA.size(); ++k) {
                    ...
                    // after you define SA you have to correct this--> in this case I assumed that the SA has as many
                    // rows as the number of elements stored in the SA (i.e., K) and as many columns as the 1 (this
                    // first element is useful for placing the penalty function values) +number of variables + number of
                    // objectives + number of iec
                    //+ number of ec which are defined in the problem --> for accessing the variables, I thus have to go
                    // from 1 (i.e., second position) to n_con (number of variables stored inside the SA) std::cout <<
                    // "individual number" << std::endl; std::cout << k << std::endl; std::cout << "SA size" <<
                    // std::endl; std::cout << SA.size() << std::endl; std::cout << "mean" << std::endl; std::cout <<
                    // SA[k][1+h] << std::endl; std::cout << "stddev" << std::endl; std::cout << sigma[h] << std::endl;
