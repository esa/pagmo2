...
void reksum(std::vector<std::vector<double>> &retval, const std::vector<population::size_type> &X,
            population::size_type m, population::size_type s, std::vector<double> eggs)
{
    if (m == 1u) {
 ...
 */
std::vector<population::size_type> non_dominated_front_2d(const std::vector<vector_double> &input_objs)
{
    ...
    std::vector<population::size_type> front;
    std::vector<population::size_type> indexes(input_objs.size());
    std::iota(indexes.begin(), indexes.end(), population::size_type(0u));
    // Sort in ascending order with respect to the first component
    std::sort(indexes.begin(), indexes.end(), [&input_objs](population::size_type idx1, population::size_type idx2) {
 ...
 *  - the non dominated fronts, an <tt>std::vector<std::vector<population::size_type>></tt>
 * containing the non dominated fronts. Example {{1,2},{3},{0}}
 *  - the domination list, an <tt>std::vector<std::vector<population::size_type>></tt>
 ...
 *  - the domination count, an <tt>std::vector<population::size_type></tt> containing the number of individuals
 * that dominate the individual at position \f$i\f$. Example {2, 0, 0, 1}
 *  - the non domination rank, an <tt>std::vector<population::size_type></tt> containing the index of the non
...
fnds_return_type fast_non_dominated_sorting(const std::vector<vector_double> &points)
{
    ...
    std::vector<std::vector<population::size_type>> non_dom_fronts(1u);
    std::vector<std::vector<population::size_type>> dom_list(N);
    std::vector<population::size_type> dom_count(N);
    std::vector<population::size_type> non_dom_rank(N);
    ...
    std::vector<std::vector<population::size_type>>::size_type front_counter(0u);
    while (current_front.size() != 0u) {
        std::vector<population::size_type> next_front;
...
vector_double crowding_distance(const std::vector<vector_double> &non_dom_front)
{
    ...
    std::vector<population::size_type> indexes(N);
    std::iota(indexes.begin(), indexes.end(), population::size_type(0u));
    vector_double retval(N, 0.);
    for (decltype(M) i = 0u; i < M; ++i) {
        std::sort(indexes.begin(), indexes.end(),
                  [i, &non_dom_front](population::size_type idx1, population::size_type idx2) {
 ...
 */
std::vector<population::size_type> select_best_N_mo(const std::vector<vector_double> &input_f, population::size_type N)
{
    if (N < 1u) {
        pagmo_throw(std::invalid_argument,
    ...
    if (N >= input_f.size()) { // corner case
        std::vector<population::size_type> retval(input_f.size());
        std::iota(retval.begin(), retval.end(), population::size_type(0u));
        return retval;
    }
    std::vector<population::size_type> retval;
    std::vector<population::size_type>::size_type front_id(0u);
    ...
    std::vector<population::size_type> idxs(front.size());
    std::iota(idxs.begin(), idxs.end(), population::size_type(0u));
    std::sort(idxs.begin(), idxs.end(), [&cds](population::size_type idx1, population::size_type idx2) {
 ...
 * @endcode
 * @code{.unparsed}
 * std::vector<population::size_type> idx(input_f.size());
 * std::iota(idx.begin(), idx.end(), population::size_type(0u));
 ...
 * @throws unspecified all exceptions thrown by pagmo::fast_non_dominated_sorting and pagmo::crowding_distance
 */
std::vector<population::size_type> sort_population_mo(const std::vector<vector_double> &input_f)
{
    ...
    std::sort(retval.begin(), retval.end(),
              [&tuple, &crowding](population::size_type idx1, population::size_type idx2) {
