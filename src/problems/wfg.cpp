...
wfg::wfg(unsigned prob_id, vector_double::size_type dim_dvs, vector_double::size_type dim_obj, vector_double::size_type dim_k)
    : m_prob_id(prob_id), m_dim_dvs(dim_dvs), m_dim_obj(dim_obj), m_dim_k(dim_k)
{
    if (dim_dvs < 1u) {
        pagmo_throw(std::invalid_argument,
                    "WFG problem suite must have minimum 1 dimension for the decision vector, " + std::to_string(dim_dvs) + " requested");
    }
    
    if (dim_obj < 2u) {
        pagmo_throw(std::invalid_argument,
                    "WFG test problems must have a minimum value of 2 for the objective vector dimension, " + std::to_string(dim_obj) + " requested");
    }

    if (dim_k >= dim_dvs || dim_k < 1 || dim_k % (dim_obj-1)!=0) {
        pagmo_throw(std::invalid_argument,
                    "WFG test problems must have a dim_k parameter which is within [1,dim_dvs), and such that dim_k mod(dim_obj-1) == 0 " + std::to_string(dim_k) + " requested");
    }
    ...
    if (prob_id == 2u || prob_id == 3u){
        if ((dim_dvs-dim_k) % 2 !=0){
            pagmo_throw(std::invalid_argument, "For problems WFG2 and WFG3 the dim_k parameter and the decision vector size must satisfy (dim_dvs-dim_k) mod(2)=0"
...
{
    return std::pow((1.0-parameters_0-std::cos(2*deg_const*boost::math::constants::pi<double>()*parameters_0 + boost::math::constants::pi<double>()/2.0)/(2.0*deg_const*boost::math::constants::pi<double>())),alpha);
}

double wfg::disconnected(const double parameters_0, const double alpha, const double beta, const double deg_const) const
{
    return 1.0 - std::pow(parameters_0,alpha)*std::pow(deg_const*std::pow(parameters_0,beta)*boost::math::constants::pi<double>(),2.0);
}
...
{
    return a_par + std::min(0.0,std::floor(y-b_par))*a_par*(b_par-y)/(b_par)-std::min(0.0,std::floor(c_par-y))*(1.0-a_par)*(y-c_par)/(1-c_par);
}
...
{
    return 1.0 + (std::abs(y-a_par)-b_par)*( (std::floor(y-a_par+b_par)*(1.0-c_par+(a_par-b_par)/b_par))/(a_par-b_par) + (std::floor(a_par+b_par-y)*(1.0-c_par+(1.0-a_par-b_par)/b_par))/(1.0-a_par-b_par) + 1.0/b_par);
}

double wfg::s_multi (const double y, const double a_par, const double b_par, const double c_par) const
{
    return (1+std::cos( (4.0*a_par+2.0)*boost::math::constants::pi<double>()*(0.5-(std::abs(y-c_par))/(2.0*(std::floor(c_par-y)+c_par)))) + 4.0*b_par*std::pow(std::abs(y-c_par)/(2*(std::floor(c_par-y) + c_par)),2) )/(b_par+2.0);
}
