...
{
    if (focus < 0. || (std::isfinite(focus) == false)) {
...
population maco::evolve(population pop) const
{
    ...
    for (decltype(m_gen) gen = 1u; gen <= m_gen; ++gen) {
        ...
        if ((m_counter == 1 && m_memory == true) || (gen == 1 && m_memory == false)) {
            ...
            unsigned front = 0u;
            // Infeasible (death-penalty) individuals skipped for hv computation are
            // kept here to pad the archive tail, so the archive stays full (m_ker)
            // when feasible individuals are not enough (see issue #525 discussion).
            std::vector<vector_double> skipped_fit;
            std::vector<vector_double> skipped_dvs;
                ...
                if (i_arch < m_ker) {
                    ...
                    for (auto idx : front_idxs) {
                        ...
                        if (valid) {
                            ...
                            skipped_fit.push_back(fit[idx]);
                            skipped_dvs.push_back(dvs[idx]);
                        }
                    }
            ...
            // Pad the archive tail with the skipped infeasible individuals so no
            // initialized (1.) entries remain when feasibles are fewer than m_ker.
            for (vector_double::size_type s = 0u; i_arch < m_ker && s < skipped_fit.size(); ++s, ++i_arch) {
                for (decltype(n_x) i_nx = 0u; i_nx < n_x; ++i_nx) {
                    sol_archive[i_arch][i_nx] = skipped_dvs[s][i_nx];
                }
                for (decltype(n_f) i_nf = 0u; i_nf < n_f; ++i_nf) {
                    sol_archive[i_arch][n_x + i_nf] = skipped_fit[s][i_nf];
                    sol_archive_fit[i_arch][i_nf] = skipped_fit[s][i_nf];
                }
            }
            if (m_memory == true) {
        ...
        if ((m_counter > 1 && m_memory == true) || (gen > 1 && m_memory == false)) {
            ...
            vector_double::size_type i_arch = 0;
            unsigned front = 0u;
            // Same tail-padding as in the first stage: keep skipped infeasible
            // individuals to fill the archive up to m_ker.
            std::vector<vector_double> skipped_fit;
            std::vector<vector_double> skipped_dvs;
            for (const auto &front_idxs : ndf) {
                if (i_arch < m_ker) {
                    ...
                    for (auto idx : front_idxs) {
                        ...
                        if (valid) {
                            ...
                            skipped_fit.push_back(merged_fit[idx]);
                            skipped_dvs.push_back(merged_dvs[idx]);
                        }
                    }
                    if (!list_of_fit.empty()) {
                        // I set-up the hypervolume computation by passing the list of fitnesses:
