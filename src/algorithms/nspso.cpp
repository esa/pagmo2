...
nspso::nspso(unsigned gen, double min_w, double max_w, double c1, double c2, double chi, double v_coeff, unsigned leader_selection_range, std::string diversity_mechanism, unsigned seed)
    : m_gen(gen), m_min_w(min_w), m_max_w(max_w), m_c1(c1), m_c2(c2), m_chi(chi), m_v_coeff(v_coeff), m_leader_selection_range(leader_selection_range), m_diversity_mechanism(diversity_mechanism), m_velocity(),
      m_e(seed), m_seed(seed), m_verbosity(0u)
{
    if (min_w <= 0 || max_w <= 0 || c1 <=0 || c2 <= 0 || chi <=0) {
        pagmo_throw(std::invalid_argument, "minimum and maximum particles' inertia weights, first and second magnitude of the force coefficients and velocity scaling factor should be greater than 0");
    }
    if (min_w>max_w) {
        pagmo_throw(std::invalid_argument, "minimum particles' inertia weight should be lower than maximum particles' inertia weight, while values of"
    ...
    if (leader_selection_range<=0 || leader_selection_range > 100) {
        pagmo_throw(std::invalid_argument, "leader selection range coefficient should be in the ]0,100] range, while a value of"
                                               + std::to_string(leader_selection_range) + " was detected");
    }
    if(diversity_mechanism != "crowding distance" && diversity_mechanism != "niche count" && diversity_mechanism != "max min") {
...
population nspso::evolve(population pop) const
{
    ...
    if (prob.get_nx() == 0u) {
        pagmo_throw(std::invalid_argument,
                    get_name()
                        + " cannot work on problems without continuous part.");
    }
    ...
    if (prob.get_nf() < 2u) {
        pagmo_throw(std::invalid_argument, "This is a multi-objective algortihm, while number of objectives detected in "
    ...
    for (decltype(m_gen) gen = 1u; gen <= m_gen; gen++) {
        ...
        if(m_diversity_mechanism == "crowding distance") {
            //This returns a std::tuple containing: -the non dominated fronts, -the domination list, -the domination count,
            ...
            double delta = 1.0;
            if (prob.get_nf() == 2) {
                delta = ( (nadir_point[0] - ideal_point[0]) + (nadir_point[1] - ideal_point[1]) ) / (non_dom_chromosomes.size()-1);
                ...
                delta = std::sqrt(4*d2*d1*ndc_size + 4*d3*d1*ndc_size + 4*d2*d3*ndc_size + std::pow(d1,2) + std::pow(d2,2) + std::pow(d3,2)
                             - 2*d2*d1 - 2*d3*d1 - 2*d2*d3 + d1 + d2 + d3) / (2*(ndc_size-1));
            ...
            std::sort(sort_list.begin(), sort_list.end(),
                      [&count](decltype(count.size()) idx1, decltype(count.size()) idx2) {
            ...
            best_non_dom_indices = std::vector<vector_double::size_type>(best_non_dom_indices.begin(), best_non_dom_indices.begin() + static_cast<vector_double::difference_type>(i)); //keep just the non dominated
        }
        ...
        for(vector_double::size_type idx = 0; idx < swarm_size; ++idx) {
            ...
            unsigned ext = static_cast<unsigned>(ceil(best_non_dom_indices.size()*m_leader_selection_range/100))-1;
        ...
        //3 - Select the best swarm_size individuals in the new population (of size 2*swarm_size) according to pareto dominance
        ...
        if(m_diversity_mechanism != "max min") {
            ...
            best_next_pop_indices = std::vector<vector_double::size_type>(sort_list_3.begin(), sort_list_3.begin() + static_cast<vector_double::difference_type>(swarm_size));

        }



        // The next_pop_list for the next generation will contain the best swarm_size individuals out of 2*swarm_size according to pareto dominance
...
{
    detail::archive(ar, m_gen, m_min_w, m_max_w, m_c1, m_c2, m_chi, m_v_coeff, m_leader_selection_range, m_diversity_mechanism, m_e, m_seed, m_verbosity, m_log);
}


double nspso::minfit(vector_double::size_type(i), vector_double::size_type(j), const std::vector<vector_double> &fit) const
...
void nspso::compute_niche_count(std::vector<vector_double::size_type> &count, const std::vector<vector_double> &chromosomes, double delta) const
