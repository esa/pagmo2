...
//                if (-0.00001 < x_i[i] - j  && x_i[i] - j < 0.00001){ // if error is small enough the item i is placed in bin j
                if ( x_i[i] == j){ // if error is small enough the item i is placed in bin j
                    bin_used[j] = 1;
                }
            }
        }
...
int main(){
    ...
    // 7 - Get the best solution across all islands. The first number in the champion vector is the number of islands used.
...
//    std::cout << champion.champion_f().size() << " " << champion.champion_x().size() << std::endl;
