...
BOOST_AUTO_TEST_CASE(bee_colony_initial_population_not_respecting_bounds_throw_test)
{
    // We test that the bee_colony algorithm throws if the initial population does not respect the bounds
    problem prob{rosenbrock{25u}};
    population pop{prob, 10u, 23u};
    auto dv = pop.get_x()[0];
    dv[0] = prob.get_bounds().first[0] - 1.;
    pop.set_x(0, dv);
    BOOST_CHECK_THROW(bee_colony{10u}.evolve(pop), std::invalid_argument);
    ...
    pop.set_x(0, dv);
    BOOST_CHECK_NO_THROW(bee_colony{10u}.evolve(pop));
}
