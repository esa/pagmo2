...
BOOST_AUTO_TEST_CASE(maco_algorithm_construction)
{
    ...
    BOOST_CHECK_THROW((maco{1u, 63u, 1.0, 1u, 7u, 10000u, nan, false, 23u}), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(maco_infeasible_archive_padding_test)
{
    // Archive tail-padding (issue #525 discussion): when feasible individuals are
    // fewer than ker, skipped death-penalty (DBL_MAX) individuals must fill the
    // archive tail so no initialized (1.) entries remain.
    struct constrained_mo_udp {
        vector_double fitness(const vector_double &x) const
        {
            vector_double f(3, 0.);
    ...
    const unsigned ker = 20u;
    const unsigned pop_size = 50u;
    const double infeas = std::numeric_limits<double>::max();
    problem prob{unconstrain{constrained_mo_udp{}, "death penalty"}};

    // All infeasible: full pad, no throw, archive all DBL_MAX.
    // Uses several generations so both the first-stage and the second-stage
    // filtering/tail-pad paths are hit: with sigma == 0 around x == 2.
    // all new ants stay infeasible.
    {
        population pop{prob, pop_size, 23u};
        for (decltype(pop.size()) i = 0u; i < pop.size(); ++i) {
            pop.set_x(i, {2.});
        }
        maco uda{3u, ker, 1.0, 1u, 7u, 10000u, 0., false, 23u};
        BOOST_CHECK_NO_THROW(pop = uda.evolve(pop));
        for (unsigned i = 0u; i < ker; ++i) {
            auto f = pop.get_f()[i];
            BOOST_REQUIRE(f.size() == 2u);
            BOOST_CHECK(f[0] == infeas);
            BOOST_CHECK(f[1] == infeas);
            BOOST_CHECK(pop.get_x()[i][0] == 2.);
        }
    }
    // Few feasible (2 < ker): partial pad, no dummy 1. entries.
    {
        population pop{prob, pop_size, 23u};
        for (decltype(pop.size()) i = 0u; i < pop.size(); ++i) {
            pop.set_x(i, {2.});
        ...
        pop.set_x(0u, {-1.});
        pop.set_x(1u, {-1.});
        maco uda{1u, ker, 1.0, 1u, 7u, 10000u, 0., false, 23u};
        BOOST_CHECK_NO_THROW(pop = uda.evolve(pop));
        unsigned n_finite = 0u, n_infeas = 0u;
        for (unsigned i = 0u; i < ker; ++i) {
            auto f = pop.get_f()[i];
            BOOST_REQUIRE(f.size() == 2u);
            if (f[0] == infeas && f[1] == infeas) {
                ++n_infeas;
                BOOST_CHECK(pop.get_x()[i][0] == 2.);
            } else {
                ++n_finite;
                BOOST_CHECK_CLOSE(f[0], 1., 1e-8);
                BOOST_CHECK_CLOSE(f[1], 4., 1e-8);
            }
            // Initialized dummy row would be x=1., f=[1., 1.].
            BOOST_CHECK(!(pop.get_x()[i][0] == 1. && f[0] == 1. && f[1] == 1.));
        }
        BOOST_CHECK(n_finite == 2u);
        BOOST_CHECK(n_infeas == ker - 2u);
    }
    // Few feasible over several generations: exercises second-stage padding.
    {
        population pop{prob, pop_size, 23u};
        for (decltype(pop.size()) i = 0u; i < pop.size(); ++i) {
            pop.set_x(i, {2.});
        ...
        pop.set_x(0u, {-1.});
        pop.set_x(1u, {-1.});
        maco uda{5u, ker, 1.0, 1u, 7u, 10000u, 0., false, 23u};
        BOOST_CHECK_NO_THROW(pop = uda.evolve(pop));
    }
}
