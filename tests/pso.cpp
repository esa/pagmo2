...
BOOST_AUTO_TEST_CASE(construction)
{
    ...
    BOOST_CHECK_THROW((pso{100, nan, 2., 2., 0.1, 5u, 2u, 4u, false, 23u}), std::invalid_argument);
    BOOST_CHECK_THROW((pso{100, 0.79, nan, 2., 0.1, 5u, 2u, 4u, false, 23u}), std::invalid_argument);
    BOOST_CHECK_THROW((pso{100, 0.79, 2., nan, 0.1, 5u, 2u, 4u, false, 23u}), std::invalid_argument);
    BOOST_CHECK_THROW((pso{100, 0.79, 2., 2., nan, 5u, 2u, 4u, false, 23u}), std::invalid_argument);
}
...
{
    // We check here that when memory is true calling evolve(pop) multiple times on 1 gen
    // is the same as calling 1 time evolve with multiple gens
    auto omega = 0.5;
    auto eta1 = 0.5;
    auto eta2 = 0.5;
    auto max_vel = 0.5;
    ...
    for (auto variant = 1u; variant <= 6; ++variant) {
        for (auto neighb_type = 1u; neighb_type <= 4; ++neighb_type) {
            ...
            // Make sure serializing and deserializing between evolutions gives the same answer
